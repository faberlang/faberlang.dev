...
Synced from `radix/EBNF_MATRIX.md` corpus-wide summary (2026-08-07). Full term
tables below may lag until the next full matrix publish.
...
- **`metal-text` and `wgsl-text` are device-kernel subset emitters.** They are
  only meant to lower a **device-safe kernel surface** (`@ nucleum` compute
  kernels and related GPU views) — not packages, async, CLI, full control
  flow, or host libraries. Scoring them against **all ~280 language terms**
  therefore yields a **tiny capable fraction by design** (today about **2%**).
  That does **not** mean Metal or WGSL “are 2% done.” Real Metal device
  execution for the accepted training path is proven via
  `faber run --backend metal` (see
  [device execution](/toolchain/cli.html#device-execution)).
- **There is no `cuda` column.** CUDA is not a separate text emit target in
  this matrix. CUDA device programs are produced on the **NVVM → PTX** path
  (staged with **llvm-text** / MIR device emission) and run with
  `faber run --backend cuda`. Product GPU backends are **Metal** and **CUDA**;
  matrix columns track **emit surfaces**, not every host session.
...
the GPU rows of this summary as a quality score.

### Application lane (HIR → emitted source languages)

| target | capable | analyzable | % | What the % means |
|---|---|---|---|---|
| rust | 278 | 280 | 99% | Full-language HIR projection |
| go | 262 | 280 | 94% | Full-language HIR projection |
| ts | 274 | 280 | 98% | Full-language HIR projection |
| faber | 280 | 280 | 100% | Canonical re-emission |

### Systems lane — general MIR (full-language IR)

| target | capable | analyzable | % | What the % means |
|---|---|---|---|---|
| llvm-text | 277 | 280 | 99% | Full-language MIR staging IR (also the **CUDA device** NVVM/PTX route — see note above) |
| wasm-text | 257 | 280 | 92% | Full-language WAT emission |
| wasm | 257 | 280 | 92% | Full-language binary Wasm emission |
| sexp-struct | 222 | 280 | 79% | Structural MIR dump |
| sexp | 222 | 280 | 79% | Validation / Racket-oriented dump |
| scena | 238 | 280 | 85% | Scena-oriented MIR surface |

### Systems lane — device kernel text (subset emitters; low % is expected)

| target | capable | analyzable | % | Read as |
|---|---|---|---|---|
| metal-text | 6 | 280 | 2% | **Kernel-subset lowerability over the full corpus** — not Metal product health. Product Metal runs: `faber run --backend metal`. |
| wgsl-text | 6 | 280 | 2% | Same idea for WebGPU shader text (browser / headless proofs), not a claim that WebGPU training is incomplete on this scale. |

| Product GPU backend | Matrix column? | How it actually runs |
|---|---|---|
| **Metal** | `metal-text` (shader text only) | `faber run --backend metal` — real MSL session |
| **CUDA** | **none** (uses llvm/NVVM device chain) | `faber run --backend cuda` — real PTX session |
| **WebGPU** | `wgsl-text` (shader text only) | Host/browser WebGPU path; not the dual-backend training product claim |
...
RunPod lanes and Faber package fixtures close. Numbers here are **evidence
snapshots**, not a permanent completion score.
...
1. The **first** multi-card receipt was largely a **per-pod compile+run
   viability** probe (same small CUDA program, pod-local toolchain), not a claim
