...
is not complete: `th-TH`, `zh-Hans`, `zh-Hant`, `vi`, `ar`, and `hi` have
portal/start fallback-proof slices plus generated corpus build artifacts, while full
...
18. **Multilingual prose: full-file-per-locale, LLM-assisted differential sync, portal at root.** Resolves how Stage 7 prose translation is actually operated — the recurring merge the *Multilingual pipeline* § below deferred. Five sub-decisions:

  **18a. Portal at root is the active path, not deferred.** `/` is the locale-less language portal (Decision 16); English content moves to `/en-US/`. Clean-break URL migration: internal links, `html.fab` sidebar hrefs, `build-site.sh` smoke checks, `inject-canonical.py`, `generate-sitemap.py`, and `check-leakage-gate.py` all move to the `/{locale}/...` scheme. Retired root URLs redirect via `<meta http-equiv="refresh">` stubs — the mechanism already used for corpus alias pages; GitHub Pages here has no `_redirects` support. Done before locale prose translation, because the sync oracle compares paths.

  **18b. Two locale namespaces; English is the sole intentional divergence.**
  - *site-locale* (the `src/{locale}/` dir name) drives the URL segment and `<html lang>`. The dir names are valid BCP-47 tags, so they double as the lang attribute with no mapping table.
  - *reader-locale* (the Faber pack id) drives `faber emit --reader-locale=` code transcoding.
  - They coincide for every locale except English: `en-US` prose + `la` code, because Faber is Latin-based and Latin keywords are the canonical code surface. Target languages match (Thai prose + Thai code, etc.). The build threads both values; `reader_locale` defaults to the dir name with `en-US` overridden to `la`. This fixes the current `<html lang="la">` on English pages, which wrongly tags English prose as Latin.

  **18c. Full-file-per-locale prose storage; reject block overlays and prose transclusion.** Each locale owns a complete `.md` file (what is built today). Page chrome is generated in `html.fab` and shared across all locales, so there is no layout to duplicate — copying a file duplicates only prose (which must exist per locale) and code fences (which regenerate from `en-US` per Decision 8). Block-level overlays and prose transclusion are rejected: they add generator merge machinery the architecture deliberately avoids (Decision 17), sever translator context, and fight the LLM workflow in 18e. The ~32 chrome strings hardcoded in `html.fab` are the one genuine piece of shared layout text; they localize via a per-locale catalog, not by duplicating Markdown.

  **18d. Two-hash provenance, not one.** Decision 15's single content-hash is refined into signals with different cadences:
  - `prose_hash` — `en-US` body with code fences stripped. Drives LLM re-translation. An example edit does *not* change this, so it does not false-flag prose re-translation.
  - `code_hash` — `en-US` fences only. Drives the build-time `faber emit` re-transcode (mechanical, free).
  - `source_commit` — the `en-US` git commit the locale was last synced against. A hash says *whether* English drifted; a commit gives the exact delta (`git diff src/en-US/X.md <source_commit>..HEAD`) to hand the LLM.
  The recorded `source_hash` in the current Stage 7 slices is already stale and its basis is undocumented — none of full-file / body / prose SHA-256 matches. Silent drift, undetected. This decision makes the basis reproducible and the drift visible.

  **18e. "Manual translation" = LLM-assisted differential re-translation, human-reviewed, fenced from code.** The Step 3 prose-translation step is operated as:
  1. A `sync-report` oracle flags locale pages whose `prose_hash`/`source_commit` lag `en-US`, with the `en-US` commits-since and a block-level diff summary.
  2. A `sync-translate` step feeds the LLM `(current en-US, current locale file, the en-US diff since source_commit)` with code fences stripped from input and output, reusing the target pack's existing `[llm]` prompt (see *Multilingual pipeline* — do not write a parallel prompt). Code is regenerated by `faber emit`, never touched by the LLM (Decision 8). The LLM preserves all structure: `{#anchor}` ids, `(/path.html)` links, table shapes, frontmatter keys. Only prose cells move.
  3. The candidate lands on a branch/worktree, not the main checkout. The human reviews the `git diff`, edits, commits. That commit bumps `prose_hash`/`code_hash`/`source_commit` to current `en-US`, clearing staleness.
  4. Gates run before merge: `validate-fences.sh`, `check-leakage-gate.py`, `check-internal-links.py`.
  Full-file storage (18c) is what makes this clean: the LLM gets whole-page context and produces a reviewable file diff. Overlays would force id-keyed fragments and discard the before/after advantage.

  → **A single locale registry** (e.g. `locales.toml` at the generator root) is the home for the dir→reader-locale override (18b), the portal's display metadata (native script and language name, today hardcoded in `render-corpus-batch.sh`), and locale status. Locale discovery stays dir-presence; the registry enriches it. One file, three consumers.

  **Cross-reference:** the *Multilingual pipeline* § below stays authoritative for the *bootstrap* (3-step fork), the `[llm]` pack prompt, and the `inherits`/`fallback` model. Decision 18 specifies the *operated recurring sync* and the locale/URL model those earlier notes assumed but did not lock. O7's "implementation later" is now specified by 18d–18e.
